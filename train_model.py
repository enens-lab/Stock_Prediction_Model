...
# --- CONFIG ---
BATCH_SIZE_PER_REPLICA = 256  # REDUCED from 512 to 256
...
class_weight = compute_class_weights(all_files, target_col='target_5d', cutoff_date=VAL_CUTOFF_DATE)
...
# Cap val set to avoid OOM in from_tensor_slices (same issue as training chunks)
...
for epoch in range(CONFIG["EPOCHS"]):
    ...
    for chunk_idx, file_chunk in enumerate(train_file_chunks):
        ...
        # CRITICAL: Limit chunk size if still too large
        max_samples = 800_000  # Hard limit per chunk
        if len(X_chunk) > max_samples:
            print(f"(Trimming from {len(X_chunk)} to {max_samples})", end=" ", flush=True)
            indices = np.random.choice(len(X_chunk), max_samples, replace=False)
        ...
        try:
            ...
            print(f"\n  ⚠️  Dataset creation failed: {e}")
            print(f"     Skipping chunk {chunk_idx + 1}")
            del X_chunk, y_chunk
            gc.collect()
            continue
