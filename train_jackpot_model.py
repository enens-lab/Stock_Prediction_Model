...
# --- CONFIG ---
BATCH_SIZE_PER_REPLICA = 512  # CHANGED: Reduced from 4096 to prevent OOM
...
# CHANGED: Removed mixed precision to prevent numerical instability
...
def process_single_file(f, feature_cols, config):
    try:
        ...
        X_raw = scaler.fit_transform(df_features.values).astype(np.float32)  # CHANGED: float32 instead of float16

        # JACKPOT TARGET: 20% gain in 20 days
...
def load_files_parallel(files, feature_cols, config, desc="Loading"):
    ...
    X = np.array(X_all, dtype=np.float32)  # CHANGED: float32 instead of float16
...
shuffle_buffer = min(len(X_train), 500_000)  # CHANGED: Reduced from 2_000_000 to save memory
...
# CHANGED: Reduced learning rates to prevent gradient explosion
lr_schedule = WarmupCosineDecay(
    warmup_steps=warmup_steps,
    total_decay_steps=cosine_decay_steps,
    initial_lr=5e-4,      # CHANGED: Reduced from 1e-3
    warmup_start_lr=1e-6,  # CHANGED: Reduced from 1e-5
)
...
with strategy.scope():
    ...
    # CHANGED: Added gradient clipping to prevent gradient explosion
...
history = model.fit(
    ...
    callbacks=[
        ...
        ModelCheckpoint(CONFIG["MODEL_PATH"], save_best_only=True, monitor='val_AUC', mode='max')
    ],
    class_weight=class_weight
)

print(f"JACKPOT MODEL SAVED: {CONFIG['MODEL_PATH']}")
